...
import { Order, orders, updateOrdersElement } from "./order-system";
...
export function addTestCat() {
  ...
  catInventory.set(
    ...
    new Cat(
      id,
      catNames[getRandomInt(catNames.length)],
      catVariants[getRandomInt(catVariants.length)]
    )
  );
}
